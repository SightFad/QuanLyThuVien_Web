...
• Tình trạng: ${
      returnSlip.condition === "good"
        ? "Tốt"
        : returnSlip.condition === "damaged"
        ? "Hư hỏng"
        : "Mất"
    }`;
...
• Đã thanh toán: ${
        finePayment.paymentMethod === "cash" ? "Tiền mặt" : "Chuyển khoản"
      }
${
  finePayment.transactionCode
    ? `• Mã giao dịch: ${finePayment.transactionCode}`
    : ""
}`;
    } else {
      successMessage += `
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  rows="2"
                  placeholder="Ghi chú của Librarian khi nhận sách..."
                />
              </div>
            </div>
          </div>
                ...
                <div className="fine-item">
                  ...
                  <span className="fine-value">
                    {Math.max(
                      0,
                      Math.floor(
                        (new Date(returnData.actualReturnDate) -

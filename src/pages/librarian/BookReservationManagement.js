...
const BookReservationManagement = () => {
  ...
  const mockReservations = [
    {
      ...
      trangThai: "Chờ xử lý",
      ghiChu: "Reader yêu cầu đặt trước",
      nguoiXuLy: null,
      ngayXuLy: null,
    },
    {
      ...
      nguoiXuLy: "Librarian Nguyễn Thị C",
      ngayXuLy: "2024-01-17",
    },
    {
      id: 3,
  ...
  const loadStatistics = async () => {
    try {
      const stats = {
        ...
        notifiedReservations: mockReservations.filter(
          (r) => r.trangThai === "Đã thông báo"
        ).length,
        completedReservations: mockReservations.filter(
          (r) => r.trangThai === "Đã nhận"
        ).length,
        cancelledReservations: mockReservations.filter(
          (r) => r.trangThai === "Đã hủy"
        ).length,
  ...
  const filterReservations = () => {
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter(
        (reservation) => reservation.trangThai === statusFilter
      );
    }
        ...
        prev.map((r) =>
          r.id === reservationId
            ? {
                ...
                ghiChu: "Đã thông báo cho Reader",
              }
            : r
        )
      );
      showToast("Đã thông báo cho Reader thành công!", "success");
    } catch (error) {
      showToast("Lỗi khi thông báo cho Reader", "error");
    }
  };
  ...
  const handleConfirmReceived = async (reservationId) => {
    try {
      ...
      showToast("Đã xác nhận Reader nhận sách!", "success");
    } catch (error) {
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-content">
            <h3>Đã thông báo</h3>
            <p className="stat-number">
              {statistics?.notifiedReservations || 0}
            </p>
          </div>
        </div>
        ...
        <div className="stat-card">
          ...
          <div className="stat-content">
            <h3>Quá hạn</h3>
            <p className="stat-number">
              {statistics?.expiredReservations || 0}
            </p>
          </div>
        </div>
      </div>
          ...
          <tbody>
            {filteredReservations.map((reservation, index) => (
              <tr
                key={reservation.id}
                className={
                  isExpired(reservation.hanLaySach) &&
              ...
              >
                ...
                <td
                  className={
                    isExpired(reservation.hanLaySach) ? "expired-date" : ""
                  }
                >
                  {formatDate(reservation.hanLaySach)}
                  {isExpired(reservation.hanLaySach) && (
                    <FaExclamationTriangle className="expired-icon" />
                  )}
                </td>
                ...
                <div className="detail-section">
                  ...
                  <div className="detail-row">
                    ...
                    <span className="value">
                      {formatDate(selectedReservation.ngayDatTruoc)}
                    </span>
                  </div>
                  <div className="detail-row">
                  ...
                  <div className="detail-row">
                    ...
                    <span className="value">
                      {getStatusBadge(selectedReservation.trangThai)}
                    </span>
                  </div>
                  ...
                  {selectedReservation.ngayXuLy && (
                    <div className="detail-row">
                      ...
                      <span className="value">
                        {formatDate(selectedReservation.ngayXuLy)}
                      </span>
                    </div>
                  )}

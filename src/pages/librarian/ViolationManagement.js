...
const ViolationManagement = () => {
  ...
  const mockViolations = [
    {
      ...
      ngayXuLy: "2024-01-20",
      nguoiXuLy: "Librarian Nguyễn Thị B",
      ghiChu: "Reader đã thanh toán đầy đủ",
    },
    {
      id: 2,
      maDG: "DG002",
    ...
    },
    {
      ...
      loaiViPham: "Mất sách",
      soNgayTre: 0,
      moTaViPham: "Reader báo mất sách",
      mucDoViPham: "Nặng",
      tienPhat: 150000,
      trangThai: "Đã phạt",
      ...
      ngayXuLy: "2024-01-25",
      nguoiXuLy: "Librarian Nguyễn Thị B",
      ghiChu: "Reader đã bồi thường 150% giá trị sách",
    },
  ];
  ...
  const loadStatistics = async () => {
    try {
      const stats = {
        ...
        pendingViolations: mockViolations.filter(
          (v) => v.trangThai === "Chưa xử lý"
        ).length,
        processedViolations: mockViolations.filter(
          (v) => v.trangThai === "Đã xử lý"
        ).length,
        finedViolations: mockViolations.filter((v) => v.trangThai === "Đã phạt")
          .length,
  ...
  const filterViolations = () => {
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter(
        (violation) => violation.trangThai === statusFilter
      );
    }
    ...
    if (violationTypeFilter !== "all") {
      filtered = filtered.filter(
        (violation) => violation.loaiViPham === violationTypeFilter
      );
    }
  ...
  const getSeverityBadge = (severity) => {
    ...
    return (
      <span
        className={`severity-badge ${config.class}`}
    ...
    <div className="violation-management">
      ...
      <div className="page-header">
        <div className="header-content">
          ...
          <p className="page-description">
            Quản lý và theo dõi các vi phạm của Reader
          </p>
        </div>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-content">
            ...
            <p className="stat-number">
              {statistics?.processedViolations || 0}
            </p>
          </div>
        </div>
        ...
        <div className="stat-card">
          ...
          <div className="stat-content">
            ...
            <p className="stat-number">
              {formatCurrency(statistics?.totalFines || 0)}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="search-filters">
        <div className="search-box">
          ...
          <input
            type="text"
            placeholder="Tìm kiếm theo tên Reader, mã sách..."
            value={searchTerm}
            ...
            {filteredViolations.map((violation, index) => (
              <tr key={violation.id}>
                ...
                <td className="fine-amount">
                  {formatCurrency(violation.tienPhat)}
                </td>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewViolation((prev) => ({
                        ...prev,
                        tenDG: e.target.value,
                      }))
                    }
                    placeholder="Nhập tên Reader"
                  />
                </div>
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewViolation((prev) => ({
                        ...prev,
                        tienPhat: parseInt(e.target.value) || 0,
                      }))
                    }
            ...
            <div className="modal-body">
              <div className="violation-detail">
                <div className="detail-section">
                  <h3>Thông tin Reader</h3>
                  <div className="detail-row">
                    <span className="label">Mã Reader:</span>
                    <span className="value">{selectedViolation.maDG}</span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Tên Reader:</span>
                ...
                <div className="detail-section">
                  ...
                  <div className="detail-row">
                    ...
                    <span className="value">
                      {getViolationTypeBadge(selectedViolation.loaiViPham)}
                    </span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Mức độ:</span>
                    <span className="value">
                      {getSeverityBadge(selectedViolation.mucDoViPham)}
                    </span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Số ngày trễ:</span>
                    <span className="value">
                      {selectedViolation.soNgayTre} ngày
                    </span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Tiền phạt:</span>
                    <span className="value fine-amount">
                      {formatCurrency(selectedViolation.tienPhat)}
                    </span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Mô tả:</span>
                    <span className="value">
                      {selectedViolation.moTaViPham}
                    </span>
                  </div>
                </div>
                ...
                <div className="detail-section">
                  ...
                  <div className="detail-row">
                    ...
                    <span className="value">
                      {getStatusBadge(selectedViolation.trangThai)}
                    </span>
                  </div>
                  <div className="detail-row">
                    <span className="label">Ngày vi phạm:</span>
                    <span className="value">
                      {formatDate(selectedViolation.ngayViPham)}
                    </span>
                  </div>
                  {selectedViolation.ngayXuLy && (
                    <div className="detail-row">
                      ...
                      <span className="value">
                        {formatDate(selectedViolation.ngayXuLy)}
                      </span>
                    </div>
                  )}

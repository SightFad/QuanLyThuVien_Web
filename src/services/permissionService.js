...
class PermissionService {
  ...
  static canPerformRole(userRole, targetRole) {
    ...
    // Admin có thể thực hiện tất cả vai trò trừ Giám đốc
    if (userRole === USER_ROLES.ADMIN && targetRole !== USER_ROLES.DIRECTOR)
      return true;
    ...
    // Trưởng thư viện có thể thực hiện vai trò Librarian
    if (
      userRole === USER_ROLES.LIBRARY_MANAGER &&
      targetRole === USER_ROLES.LIBRARIAN
    )
      return true;

    // Trưởng phòng Accountant có thể thực hiện vai trò nhân viên Accountant
    if (
      userRole === USER_ROLES.ACCOUNTING_MANAGER &&
      targetRole === USER_ROLES.ACCOUNTANT
    )
      return true;
  ...
  static getRoleDescription(userRole) {
    const descriptions = {
      ...
      [USER_ROLES.LIBRARY_MANAGER]:
        "Quản lý Librarian, duyệt đề xuất bổ sung sách",
      [USER_ROLES.LIBRARIAN]: "Quản lý mượn/trả sách, đăng ký Reader",
      [USER_ROLES.ACCOUNTING_MANAGER]:
        "Duyệt đề xuất mua sách, quản lý Accountant",
